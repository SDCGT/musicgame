...
public class AMDFSystem : ComponentSystem
{
     ...
     * 在OnCreate方法中,通过定义EntitiesQueryDesc来查询组件PitchDetector，并将它赋值给pitchDetectors变量。它还实例化了一个AMDF对象，并将采样率和采样窗口长度作为参数传入。
     * 在OnUpdate方法中，使用Entities.ForEach循环遍历所有具有PitchDetector组件的实体。
     * 在循环体中，首先检查播放器是否为空，如果是，直接返回。然后从播放器/麦克风中获取一个长度为1024的音频缓冲区，并将其传递给AMDF对象的getPitch()方法。
