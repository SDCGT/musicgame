...
{
    //这个类有三个构造函数，其中一个是默认构造函数，另一个是带有音频采样率和缓冲区大小参数的构造函数，第三个是带有音频采样率，缓冲区大小和YIN阈值参数的构造函数。
    //其中音频采样率指的是音频信号的采样频率，缓冲区大小指的是算法处理的音频块的大小，YIN阈值是用来确定哪些峰值被视为音调候选的参数。

    //这个类还定义了一些私有变量和方法，如yinBuffer，audioBufferFFT，kernel，yinStyleACF，fft和result，这些变量和方法用于实现YIN算法的不同步骤。
    //最重要的方法是getPitch()方法，它接收音频缓冲区作为参数，并返回一个PitchDetectionResult对象，该对象包含检测到的音调值。
